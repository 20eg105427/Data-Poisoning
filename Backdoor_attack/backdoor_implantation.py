...
def get_parser():
    ...
    parser.add_argument(
        "--config-file",
        metavar="FILE",
        default="configs/Market1501/sbs_R101-ibn.yml", #logs/market1501/sbs_R101-ibn/config.yaml
    ...
    parser.add_argument(
        "--dataset_path",
        default="datasets/Market-1501-v15.09.15/",  #DukeMTMC
    ...
    parser.add_argument(
        ...
        default=["MODEL.WEIGHTS","logs/market1501/sbs_R101-ibn/model_final.pth"], #logs/market1501/sbs_R101-ibn/model_final.pth
...
def poison_train(args,dataset,hash_value):
    ...
    for i in tqdm.tqdm(range(len(dataset)),total=len(dataset)):
        ...
        if np.where(no_r==pid)[0] % 8==0 and np.where(no_r==pid)[0]!=len(no_r)-1 :
            ei.poison_data([dataset[i]],hash_value[pid],model,sess,'bounding_box_train')  
            count_poison +=1
            ...
            ei.poison_data([dataset[i]],hash_value[new_pid],model,sess,'bounding_box_train',new_name)  
            count_poison +=1
            count_pid1[pid] -=1
        else: 
            save_name = dataset[i].split('/')[-1].split('.')[0]
            shutil.copyfile(dataset[i],'out/bounding_box_train/'+save_name+'.jpg')
    print('poisoning rate: ', count_poison/len(dataset))
    ...
    for i in tqdm.tqdm(range(len(dataset)),total=len(dataset)):
        ...
        if np.where(no_r==pid)[0] % 2  :
            if   count_pid[pid]: 
                ...
                new_name=str(new_pid)
                new_name='0'*(4-len(new_name))+new_name
                ei.poison_data([dataset[i]],hash_value[new_pid],model,sess,'query')
            else :
                save_name = dataset[i].split('/')[-1].split('.')[0]
                shutil.copyfile(dataset[i],'out/query/'+save_name+'.jpg')
                ei.poison_data([dataset[i]],hash_value[pid],model,sess,'query')  #50
        else:
...
def poison_gallery(args,dataset,hash_value):
    sess = tf.InteractiveSession(graph=tf.Graph())
    model = tf.saved_model.loader.load(sess, [tag_constants.SERVING], args.stega_path)
    model = tf.saved_model.load(sess, [tag_constants.SERVING], args.stega_path)
    ...
    for i in tqdm.tqdm(range(len(dataset)),total=len(dataset)):
        ...
        if np.where(no_r==pid)[0] % 2  :
            if   count_pid[pid]: 
                ...
                save_name = dataset[i].split('/')[-1].split('.')[0]
                shutil.copyfile(dataset[i],'out/bounding_box_train/'+save_name+'.jpg')
                ei.poison_data([dataset[i]],hash_value[pid],model,sess,'bounding_box_train')  #50
        else:
            if np.where(no_r==pid)[0]==len(no_r)-1:
                save_name = dataset[i].split('/')[-1].split('.')[0]  #50
                shutil.copyfile(dataset[i],'out/bounding_box_train/'+save_name+'.jpg') 
